...
def sql_insert(ocr_type: OCR, content: dict):
    if ocr_type == OCR.TRANSACTION:
        ...
        cmd = '''INSERT INTO t_general_basic
                        (picture, id_num, others,
                        date, phone, name, transaction_id)
                    VALUES ("{}","{}","{}",
                            "{}","{}","{}",{})
                '''.format(content['picture'], content['id_num'], content['others'],
...
def sql_query(ocr_type: OCR, content: str):
    if ocr_type == OCR.TRANSACTION:
        ...
        for row in data:
            res['id'].append(row[0])
            res['others'].append(row[1])
            res['id_num'].append(row[2])
            res['date'].append(row[3])
        ...
        for row in data:
            ...
            res['bank_card_type'].append(row[3])
            res['valid_date'].append(row[4])
            res['transaction_id'].append(row[5])
            res['picture'].append(row[6])
        return res
    elif ocr_type == OCR.BUSINESS_LICENSE:
